...
public class FUNCADICIONAL2Table extends GenericTable {
	...
	private final String tableName = "ERF inner join (select distinct sep_epg_codigo as codigodoempregado, min(sep_data) as mindata from erf group by emp_codigo, sep_epg_codigo) on codigodoempregado = sep_epg_codigo and  mindata = sep_data where emp_codigo = '0008' and eve_codigo <> '011' order by emp_codigo, codigodoempregado, mindata";
	...
	public FUNCADICIONAL2Table(Connector con) {
		...
		try {
			while(rs.next()) {
				...
				}else if (evecodigoread == 100) {
					dbl.getFields().add(new StringField(8, rs.getString("PARAMETRO")));	
					} else if (evecodigoread == 212) {
						dbl.getFields().add(new StringField(8, rs.getString("PARAMETRO")));	
					} else if (evecodigoread == 905) {
					dbl.getFields().add(new StringField(8, rs.getString("PARAMETRO")));
				} else {
