	...
	public FUNCCARGOTable(Connector con) {
		...
		try {
			while(rs.next()) {
				...
				dbl.getFields().add(new StringField(7, rs.getString("EPG_CODIGO")));
				dbl.getFields().add(new DateField(10, rs.getTimestamp("ADMISSAODATA")));
				dbl.getFields().add(new StringField(7, rs.getString("CAR_CODIGO")));
				dbl.getFields().add(new StringField(4, "0001"));
				dbl.getFields().add(new StringField(3, "009"));
