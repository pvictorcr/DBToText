...
public class FUNCSINDICATOTable extends GenericTable {
	...
	public FUNCSINDICATOTable(Connector con) {
		...
		try {
			while(rs.next()) {
				...
				dbl.getFields().add(new NumberField(5, rs.getInt("SIN_CODIGO")));
				...
				dbl.getFields().add(new NumberField(1, "0"));
				dbl.getFields().add(new NumberField(1, "0")); //desconta contribuicao sindical
				dbl.getFields().add(new NumberField(1, "0"));
